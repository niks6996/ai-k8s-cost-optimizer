...
class MetricsHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        ...
        try:
            payload = collect_optimizer_metrics(namespace).encode("utf-8")
            status = 200
        except Exception as exc:
            payload = (
                "# HELP optimizer_metrics_collection_error "
                "Whether the latest metrics collection failed.\n"
...
def main() -> None:
    # Binding to all interfaces is intentional inside the Kubernetes Pod.
    # The Service must reach this process on the Pod network.
    host = os.getenv("METRICS_HOST", "0.0.0.0")  # nosec B104
